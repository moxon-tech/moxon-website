(async function initMoxonAdmin() {
  ...
  const fetchSupabasePublicRows = async (path, timeoutMs = 7000) => {
    const baseUrl = String(supabaseConfig.url || "").replace(/\/+$/, "");
    const anonKey = supabaseConfig.anonKey || "";
    if (!baseUrl || !anonKey) throw new Error("Thiếu cấu hình Supabase.");
  ...
  const loadTableWithSdkFallback = async (restPath, sdkQuery, label) => {
    try {
      return await fetchSupabasePublicRows(restPath);
    } catch (restError) {
      console.warn(`Không tải được ${label} bằng REST, thử Supabase SDK.`, restError);
    }

    try {
      const result = await withTimeout(sdkQuery(), 7000, label);
      if (result.error) throw result.error;
      return result.data || [];
    } catch (sdkError) {
      console.warn(`Không tải được ${label} bằng Supabase SDK.`, sdkError);
      return null;
    }
  };
  ...
  const loadRemoteCatalogData = async () => {
    ...
    const [categoriesRows, productsRows, cmsRows] = await Promise.all([
      loadTableWithSdkFallback(
        "product_categories?select=*&order=sort_order.asc",
        () => supabaseClient.from("product_categories").select("*").order("sort_order", { ascending: true }),
        "product_categories"
      ),
      loadTableWithSdkFallback(
      ...
      loadTableWithSdkFallback(
        "cms_sections?select=*",
        () => supabaseClient.from("cms_sections").select("*"),
        "cms_sections"
      )
    ]);
  ...
  const loadRemoteContactMessages = async () => {
    ...
    try {
      ...
      if (messagesResult.error) {
        console.warn("Không tải được contact_messages từ Supabase.", messagesResult.error);
        return getData().contactMessages || [];
      }
      return (messagesResult.data || []).map(mapMessageFromDb);
    } catch (error) {
  ...
  const loadRemoteActivityLogs = async () => {
    ...
    if (result.error) {
      console.warn("Không tải được nhật ký hoạt động từ Supabase.", result.error);
      activityLogs = [];
      return activityLogs;
    }
    activityLogs = (result.data || []).map(mapActivityLogFromDb);
    return activityLogs;
  };
