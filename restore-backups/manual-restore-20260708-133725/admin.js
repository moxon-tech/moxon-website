(async function initMoxonAdmin() {
  ...
  const fetchSupabaseRest = async (table, query = "select=*", options = {}) => {
    const baseUrl = String(supabaseConfig.url || "").replace(/\/+$/, "");
    const anonKey = supabaseConfig.anonKey || "";
    if (!baseUrl || !anonKey) {
      throw new Error("Thiếu URL hoặc anon key Supabase.");
    }
    const authToken = options.auth ? sessionData.session?.access_token || anonKey : anonKey;
  ...
  const loadRemoteCatalogData = async () => {
    ...
    remoteLoadErrors = [];
    remoteAuthErrors = [];
    const readTable = async (table, query = "select=*") => {
      try {
        const needsAuth = table === "contact_messages" || table === "admin_activity_logs";
        const data = await fetchSupabaseRest(table, query, { auth: needsAuth });
        return { data, error: null, source: "rest" };
      } catch (restError) {
        return { data: [], error: restError };
      }
    };
    const [categoriesResult, productsResult, cmsResult] = await Promise.all([
      readTable("product_categories"),
      readTable("products"),
      readTable("cms_sections")
    ]);
    if (categoriesResult.error) {
      remoteLoadErrors.push(`product_categories: ${categoriesResult.error.message || "không tải được danh mục"}`);
      console.warn("Không tải được product_categories từ Supabase.", categoriesResult.error);
    }
    if (productsResult.error) {
      remoteLoadErrors.push(`products: ${productsResult.error.message || "không tải được sản phẩm"}`);
      console.warn("Không tải được products từ Supabase.", productsResult.error);
    }

    const cmsData = {};
    if (cmsResult.error) {
      remoteLoadErrors.push(`cms_sections: ${cmsResult.error.message || "không tải được nội dung CMS"}`);
      console.warn("Không tải được cms_sections. Hãy chạy supabase-cms-schema.sql.", cmsResult.error);
    } else {
      (cmsResult.data || []).forEach((row) => {
    ...
    return {
      ...fallbackData,
      ...cmsData,
      productCategories: categoriesResult.error
        ? getData().productCategories || []
        : (categoriesResult.data || []).map(mapCategoryFromDb).sort((a, b) => (Number(a.sortOrder) || 0) - (Number(b.sortOrder) || 0)),
      products: productsResult.error
        ? getData().products || []
        : (productsResult.data || []).map(mapProductFromDb).sort((a, b) => (Number(a.sortOrder) || 0) - (Number(b.sortOrder) || 0)),
  ...
  const loadRemoteContactMessages = async () => {
    try {
      const rows = await fetchSupabaseRest("contact_messages", "select=*&order=created_at.desc", { auth: true });
      return (rows || [])
        .map(mapMessageFromDb)
        .sort((a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime());
    } catch (error) {
      remoteAuthErrors.push(`contact_messages: ${error.message || "không tải được liên hệ"}`);
      console.warn("Không tải được contact_messages từ Supabase.", error);
      return getData().contactMessages || [];
    }
  };
  ...
  const syncRemoteSection = async (sectionKey, sectionValue) => {
    ...
    if (sectionKey === "contactMessages") {
      const currentResult = await supabaseClient.from("contact_messages").select("id");
      if (currentResult.error) throw currentResult.error;
      const nextRecords = Array.isArray(sectionValue) ? sectionValue : [];
      const nextIds = new Set(nextRecords.map((record) => record.id));
      const oldIds = (currentResult.data || []).map((record) => record.id);
      const deletedIds = oldIds.filter((id) => !nextIds.has(id));

      for (const id of deletedIds) {
        const deleteResult = await supabaseClient.from("contact_messages").delete().eq("id", id);
        if (deleteResult.error) throw deleteResult.error;
      }

      if (nextRecords.length) {
        const upsertResult = await supabaseClient.from("contact_messages").upsert(nextRecords.map(mapMessageToDb), { onConflict: "id" });
        if (upsertResult.error) throw upsertResult.error;
      }
    ...
    const currentResult = await supabaseClient.from(table).select("id");
    if (currentResult.error) throw currentResult.error;
    ...
    for (const id of deletedIds) {
      const deleteResult = await supabaseClient.from(table).delete().eq("id", id);
      if (deleteResult.error) throw deleteResult.error;
    }

    if (nextRecords.length) {
      const upsertResult = await supabaseClient.from(table).upsert(nextRecords.map(mapper), { onConflict: "id" });
      if (upsertResult.error) throw upsertResult.error;
    }
  ...
  const addActivityLog = async (action, target, detail) => {
    ...
    if (error) {
      console.warn("Không lưu được nhật ký hoạt động lên Supabase.", error);
      showToast(`Dữ liệu đã lưu, nhưng nhật ký hoạt động chưa ghi được: ${error.message || "kiểm tra bảng admin_activity_logs"}.`, "error");
      return null;
    }
  ...
  const loadRemoteActivityLogs = async () => {
    ...
    let remoteLogs = [];
    try {
      remoteLogs = await fetchSupabaseRest("admin_activity_logs", "select=*&order=created_at.desc&limit=50", { auth: true });
    } catch (restError) {
      console.warn("Không tải được nhật ký hoạt động từ Supabase.", restError);
      remoteAuthErrors.push(`admin_activity_logs: ${restError.message || "không tải được nhật ký"}`);
      activityLogs = [];
      return activityLogs;
    }
    ...
    activityLogs = remoteLogs.map(mapActivityLogFromDb);
    return activityLogs;
  };
  ...
  let data = deepClone(fallbackData);
  let activityLogs = [];
  let remoteLoadErrors = [];
  let remoteAuthErrors = [];
  ...
  const saveSectionData = async (sectionKey, sectionValue, actionInfo = null) => {
    ...
    let savedData;
    if (supabaseClient && remoteSectionKeys.has(sectionKey)) {
  ...
  const syncAboutPageForm = async (section) => {
    ...
    try {
      ...
      status.textContent = supabaseClient ? "Đã lưu thay đổi vào Supabase." : "Chưa kết nối Supabase.";
  ...
  const syncBannerForm = async (form) => {
    ...
    try {
      ...
      status.textContent = supabaseClient ? "Đã lưu banner vào Supabase." : "Chưa kết nối Supabase.";
      status.style.color = "#138a5b";
      afterListSave(
  ...
  const syncProductForm = async (form) => {
    ...
    try {
      ...
      status.textContent = supabaseClient ? "Đã lưu sản phẩm vào Supabase." : "Chưa kết nối Supabase.";
      status.style.color = "#138a5b";
      afterListSave(
  ...
  const syncServiceForm = async (form) => {
    ...
    try {
      ...
      status.textContent = supabaseClient ? "Đã lưu dịch vụ vào Supabase." : "Chưa kết nối Supabase.";
      status.style.color = "#138a5b";
      afterListSave(
  ...
  const syncListForm = async (form) => {
    ...
    try {
      ...
      status.textContent = `Đã lưu ${config.singular} vào ${remoteSectionKeys.has(key) && supabaseClient ? "Supabase" : "bộ nhớ phiên hiện tại"}.`;
      status.style.color = "#138a5b";
  ...
  const renderDashboardV2 = () => {
    ...
    const connectionWarning = remoteLoadErrors.length || remoteAuthErrors.length
      ? `
        <div class="admin-form-status" style="display:block; margin: 0 0 16px; color:#b45309; background:#fffbeb; border:1px solid #fcd34d; border-radius:8px; padding:10px 12px;">
          ${remoteLoadErrors.length
            ? `Không tải đủ dữ liệu public từ Supabase: ${escapeHtml(remoteLoadErrors.join(" | "))}.`
            : ""}
          ${remoteAuthErrors.length
            ? ` Dữ liệu quản trị cần đăng nhập lại hoặc làm mới phiên Supabase: ${escapeHtml(remoteAuthErrors.join(" | "))}.`
            : ""}
          <button type="button" class="admin-link-btn" data-supabase-diagnostic style="margin-left: 10px; color:#047857;">Kiểm tra kết nối</button>
          ${remoteAuthErrors.length ? `<button type="button" class="admin-link-btn" data-auth-reset style="margin-left: 10px; color:#b91c1c;">Đăng nhập lại</button>` : ""}
        </div>
      `
      : "";
    ...
    editor.querySelector("[data-auth-reset]")?.addEventListener("click", async () => {
      try {
        await supabaseClient.auth.signOut();
      } catch (error) {
        console.warn("Không đăng xuất được phiên Supabase hiện tại, chuyển về login.", error);
      }
      window.location.href = "login.html";
    });

    editor.querySelector("[data-supabase-diagnostic]")?.addEventListener("click", async (event) => {
      const button = event.currentTarget;
      button.disabled = true;
      button.textContent = "Đang kiểm tra...";
      try {
        const publicRows = await fetchSupabaseRest("products", "select=id,title&limit=1", { auth: false });
        let authMessage = "chưa kiểm tra auth";
        try {
          const authRows = await fetchSupabaseRest("contact_messages", "select=id&limit=1", { auth: true });
          authMessage = `auth OK (${Array.isArray(authRows) ? authRows.length : 0} dòng)`;
        } catch (authError) {
          authMessage = `auth lỗi: ${authError.message || authError}`;
        }
        showToast(`Supabase public OK (${Array.isArray(publicRows) ? publicRows.length : 0} dòng), ${authMessage}`, "success");
      } catch (error) {
        showToast(`Supabase vẫn lỗi: ${error.message || error}`, "error");
      } finally {
  ...
  const wireEditorActions = (section) => {
    ...
    if (listFormElement) {
      ...
      listFormElement.onsubmit = (event) => {
        event.preventDefault();
        syncListForm(event.currentTarget);
      };
    }
    ...
    if (bannerFormElement) {
      bannerFormElement.onsubmit = (event) => {
        event.preventDefault();
        syncBannerForm(event.currentTarget);
      };
    }
    ...
    if (productFormElement) {
      productFormElement.onsubmit = (event) => {
        event.preventDefault();
        syncProductForm(event.currentTarget);
      };
    }
    ...
    if (serviceFormElement) {
      serviceFormElement.onsubmit = (event) => {
        event.preventDefault();
        syncServiceForm(event.currentTarget);
      };
    }
  ...
  (async () => {
    if (supabaseClient) {
      try {
        const remoteCatalogData = await loadRemoteCatalogData();
    ...
    try {
      const remoteMessages = await loadRemoteContactMessages();
    ...
    // Vẽ lại sau khi thử tải dữ liệu quản trị.
    render();
  })();
})();
